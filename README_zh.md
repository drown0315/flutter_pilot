...
`recording: {}` 会启用默认 Scenario Recording。省略 `recording` 表示不录制，
也可以用 `recording.enabled: false` 显式禁用。`recording: true` 这类布尔简写
是无效 YAML 形态。

HTML timeline report 会把同一次 UI 旅程转换成可视化审查界面：

![Flutter Pilot HTML timeline report 示例](docs-internal/assets/timeline-report-example.svg)
...
完整语法、示例和校验规则见 [docs-internal/scenario-yaml.md](docs-internal/scenario-yaml.md)。
...
- [docs-internal/scenario-yaml.md](docs-internal/scenario-yaml.md)：Scenario YAML 语法。
- [docs-internal/flutter-pilot-prd.md](docs-internal/flutter-pilot-prd.md)：产品范围和实现决策。
- [docs-internal/test-command-prd.md](docs-internal/test-command-prd.md)：`test` 命令的启动、
...
- [docs-internal/run-diff.md](docs-internal/run-diff.md)：Run Diff 命令、输出、回归规则和验收
  测试用例。
