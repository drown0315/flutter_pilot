...
| 13. Compare Before/After Run Directories | `#13`, `#27` | In progress | `diff <before-run> <after-run>` now compares Step outcomes, run-level visible text, and runtime failures from `run_report.json`, diagnostic summaries, Snapshot artifacts, and Logs artifacts. It prints human-readable output or `--json`, reports warnings, Regressions, resolved Steps, missing Steps, added Steps, action changes, visible text changes, resolved runtime failures, new runtime failure Regressions, detailed Step identities, and overall outcomes, and preserves exit code `0` for successful diff generation even with Regressions. Screenshot hash reporting and final documentation remain incomplete. |
