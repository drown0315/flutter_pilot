...
| 4. Replace Run With Test Command | `#85` | In progress | The public CLI shell registers `test` instead of `run`, exposes `--device` / `-d`, `--flavor`, `--target` / `-t`, `--until`, repeated `--print`, and `--json`, and keeps Scenario validation plus `--until` / `--print` errors before app launch. The default executor now discovers Target Devices when needed, launches the Target App Package, creates the Runtime Target from `app.debugPort.wsUri`, runs `ScenarioRunner`, wires Scenario Recording to the resolved Target Device id, and cleans up the launched Flutter process after runner completion. Remaining work includes explicit interrupted-run cleanup coverage before marking #85 complete. Verified this phase with `dart format .`, `dart analyze`, and `dart test`. |
