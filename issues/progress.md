...
| 5. Record Target Device Metadata In Run Reports | `#86` | Complete | Run reports can now include Target Device id, name, Flutter target platform, emulator flag, and sdk string without exposing Flutter `isSupported` or Recording Device backend objects. The `test` executor passes the resolved Target Device into `ScenarioRunner`, run directory names remain unchanged, and Target App Launch Progress shows the resolved Target Device when metadata is present. Report tests cover metadata presence and absence. Verified with `dart analyze` and targeted runner/executor tests. |
