...
| 13. Compare Before/After Run Directories | `#13`, `#27` | In progress | Minimal `diff <before-run> <after-run>` now compares Step outcomes from `run_report.json`, prints a human-readable Run Diff, reports warnings, Regressions, resolved Steps, missing Steps, added Steps, and action changes, and preserves exit code `0` for successful diff generation even with Regressions. JSON output, visible text/runtime failure diffing, screenshot hash reporting, and final documentation remain incomplete. |
