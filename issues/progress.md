...
| 4. Replace Run With Test Command | `#85` | In progress | The public CLI shell now registers `test` instead of `run`, exposes `--device` / `-d`, `--flavor`, `--target` / `-t`, `--until`, repeated `--print`, and `--json`, and keeps Scenario validation plus `--until` / `--print` errors before app launch. The remaining work is to wire Target Device discovery, Target App launch, Runtime Target creation, ScenarioRunner execution, Scenario Recording ordering, and cleanup. Verified this phase with `dart analyze` and `dart test test/cli_test.dart`. |
