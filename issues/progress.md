...
| 13. Compare Before/After Run Directories | `#13`, `#27` | In progress | `diff <before-run> <after-run>` now compares Step outcomes from `run_report.json`, prints human-readable output or `--json`, reports warnings, Regressions, resolved Steps, missing Steps, added Steps, action changes, detailed Step identities, and overall outcomes, and preserves exit code `0` for successful diff generation even with Regressions. Visible text/runtime failure diffing, screenshot hash reporting, and final documentation remain incomplete. |
