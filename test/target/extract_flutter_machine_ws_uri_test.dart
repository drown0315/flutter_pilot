...
''');
  ...
  test('also supports object-shaped machine events', () async {
    final Directory tempDirectory = Directory.systemTemp.createTempSync(
      'flutter_machine_uri_',
    );
    try {
      final File logFile = File('${tempDirectory.path}/machine.log')
        ..writeAsStringSync('''
{"event":"app.debugPort","params":{"wsUri":"ws://127.0.0.1:5678/token=/ws"}}
''');
      ...
      expect(
        result.stdout,
        contains('vm_service_uri=ws://127.0.0.1:5678/token=/ws'),
      );
    } finally {
      tempDirectory.deleteSync(recursive: true);
    }
